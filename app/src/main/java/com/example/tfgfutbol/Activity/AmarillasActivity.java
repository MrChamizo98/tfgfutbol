...
public class AmarillasActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.amarillas_layout);

        barChart=(BarChart) findViewById(R.id.barchartamarillas);
        ...
        BDCall(new OnGetDataListener() {
            ...
            public void onSuccess(DataSnapshot dataSnapshot) {
                for (int i=0;i<alineaciones.length;i++){
                    ...
                    if (jugador.getAlineacion_amarilla()!="0"){
                        for (int j=0;j<equipos.size();j++){
                            if(jugador.getAlineacion_equipo().equals(equipos.get(j).getEquipo())){
                                int gol=Integer.parseInt(jugador.getAlineacion_amarilla());
