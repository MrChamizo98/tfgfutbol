...
public class RojasActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.rojas_layout);

        barChart=(BarChart) findViewById(R.id.barchartrojas);
        ...
        BDCall(new OnGetDataListener() {
            ...
            public void onSuccess(DataSnapshot dataSnapshot) {
                for (int i=0;i<alineaciones.length;i++){
                    ...
                    if (jugador.getAlineacion_roja()!="0"){
                        for (int j=0;j<equipos.size();j++){
                            if(jugador.getAlineacion_equipo().equals(equipos.get(j).getEquipo())){
                                int gol=Integer.parseInt(jugador.getAlineacion_roja());
